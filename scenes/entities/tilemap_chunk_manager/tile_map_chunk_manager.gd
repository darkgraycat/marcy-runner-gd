...
func _ready() -> void:
	if not tile_map_chunk_editor_hint:
		push_error("TileMapChunkEditorHint is required")
		return
	...
	for layer: TileMapLayer in tilemap_layers:
		tilemap_patterns[layer.name] = tile_map_chunk_editor_hint.get_tilemap_layer_patterns(layer)
		layer.clear()
...
func apply_pattern(idx: int, pos: Vector2i) -> void:
	for layer: TileMapLayer in tilemap_layers:
		...
		var chunk_size := tile_map_chunk_editor_hint.get_tilemap_layer_chunk_size(layer)
