...
func parse(prop_name: StringName, value: Variant, type: int) -> void:
	...
	if current is Array and value is Array: Util.cast_array(current, value, type)
	else: self[prop_name] = Util.cast_type(value, type)

func save(data: Dictionary) -> void:
	Util.save_as_text(json.resource_path, JSON.stringify(data))
